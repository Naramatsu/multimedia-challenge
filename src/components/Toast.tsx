...
const Toast = ({
  ...
  return (
    <section
      role="alert"
      className={`w-[350px] min-h-16 shadow-lg rounded-lg flex justify-start items-center gap-2 p-4 border-2 text-white fixed bottom-8 right-8 text-left
    ...
    >
      {type === ToastTypes.SUCCESS && (
        <FaRegCheckCircle size={32} className="min-h-8 min-w-8" />
      )}
      {type === ToastTypes.ERROR && (
        <FaInfoCircle size={32} className="min-h-8 min-w-8" />
      )}
